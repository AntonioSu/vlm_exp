#!/usr/bin/env bash
# Wait for optional training PID / free GPU, then fill missing formal M2
# offline evals (Geo3K + text) for available every-10 checkpoints.
#
# Usage:
#   WAIT_PID=<train_pid> bash scripts/eval/supplement_m2_eval.sh
#   ONLY_STEPS="50 100 150" CUDA_VISIBLE_DEVICES=0 bash scripts/eval/supplement_m2_eval.sh
set -euo pipefail
...
# M2 may still be mid-run when supplementing intermediate steps.
...
echo "=== M2 offline eval ONLY_STEPS=${ONLY_STEPS} at $(date -Is) ==="
ALLOW_INCOMPLETE_EXPS="${ALLOW_INCOMPLETE_EXPS}" \
  ONLY_EXPS=m2_mix50_2b \
...
echo "M2 supplement complete at $(date -Is)"
