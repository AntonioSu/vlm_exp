#!/usr/bin/env bash
# Wait for optional training PID / free GPU, then fill missing formal M3
# offline evals (Geo3K + text) for available every-10 checkpoints.
#
# Usage:
#   WAIT_PID=<train_pid> bash scripts/eval/supplement_m3_eval.sh
#   ONLY_STEPS="10 20 50 100 150" CUDA_VISIBLE_DEVICES=0 bash scripts/eval/supplement_m3_eval.sh
set -euo pipefail
...
# M3 is often mid-run; skip missing actors on the default grid.
...
echo "=== M3 offline eval ONLY_STEPS=${ONLY_STEPS} at $(date -Is) ==="
ALLOW_INCOMPLETE_EXPS="${ALLOW_INCOMPLETE_EXPS}" \
  ONLY_EXPS=m3_mix20_2b \
...
echo "M3 supplement complete at $(date -Is)"
